...
def angle_at_joint(
    ...
    theta = degrees(arccos( (v1 . v2) / (|v1| |v2|) )), v1 = a - b, v2 = c - b.
    """
...
def angle_between(v1: np.ndarray | tuple[float, float], v2: np.ndarray | tuple[float, float]) -> float:
    """Angle between two vectors, in degrees: degrees(arccos( (v1 . v2) / (|v1| |v2|) ))."""
...
def angle_to_vertical(v: np.ndarray | tuple[float, float]) -> float:
    """Angle of vector v away from the image vertical axis, in degrees; 0 == vertical."""
...
def angle_to_horizontal(v: np.ndarray | tuple[float, float]) -> float:
    """Angle of vector v away from the image horizontal axis, in degrees; 0 == flat."""
    v = np.asarray(v, dtype=float)
    norm = float(np.linalg.norm(v))
    if norm <= 1e-9:
        return float("nan")
