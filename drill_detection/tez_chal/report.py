...
# Adapted from slow_march / kadam_tal report.py; only the parameter labels and the
# per-key-frame wording differ, and there is no mandatory-gate note (tez chal scores
# every parameter proportionally with no hard gate).
IMAGE_COL_WIDTH = 2.0 * inch
SCORE_COL_WIDTH = 1.6 * inch
PARAM_COL_WIDTH = 2.4 * inch
IMAGE_MAX_HEIGHT = 2.4 * inch

# --- Overall verdict / grading (single source of truth; no magic numbers in logic) ---
GRADE_BANDS = [
...
PER_STEP_STANDARD = 5.0        # a key frame "meets the standard" at >= this /10
...
def _load_metadata(results: dict, metadata: ReportMetadata | None) -> ReportMetadata:
    ...
    return ReportMetadata(drill_type="tez_chal")
...
def _parameter_cell(score: dict, styles) -> Paragraph:
    hands = score.get("hands_detected", 0)
    hand_note = "" if hands else " <font color='red'>(no hand)</font>"
    body = (
        f"Arms Straight: {score['arms_straight']:.2f}/10<br/>"
        f"Legs Straight: {score['legs_straight']:.2f}/10<br/>"
        f"Fist Closed: {score['fist_closed']:.2f}/10{hand_note}"
    )
    return Paragraph(body, styles["CellBody"])
...
def generate_pdf_report(
    ...
    for frame in key_frames:
        ...
        image_cell = (
            ...
            if image_path
            else Paragraph("Image not found", styles["CellBody"])
        )
        score = frame["score"]
        score_cell = _score_cell(rank, frame["frame_index"], score["total"], styles)
