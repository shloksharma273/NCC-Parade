...
# Structure mirrors kadam_tal/report.py, adapted for baju_swing parameters.
...
def _load_metadata(results: dict, metadata: ReportMetadata | None) -> ReportMetadata:
    ...
    return ReportMetadata(drill_type="baju_swing")
...
def generate_pdf_report(
    ...
    key_frames = results.get("key_frames", [])
    summary = results.get("summary", {})
    iteration_count = summary.get("iteration_count", len(key_frames))
    average_score = summary.get("average_score_per_swing", 0.0)
    ...
    story = _header_story(
        ...
        styles,
        extra_lines=[
            f"<b>Swing Count (iterations):</b> {iteration_count}",
